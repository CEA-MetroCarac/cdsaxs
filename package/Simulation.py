...
class StackedTrapezoidGeometry:
    ...
    def __init__(self, xp=np, from_fitter=False, initial_guess= None):
        ...
        self.ntrapezoid = len(self.initial_guess['langles']['value'])
            ...
            for key in initial_guess.keys():
                if key == 'heights':
                    if not height_is_list:
                        ...
                        variations = self.xp.concatenate( (variations, [ initial_guess[key]['variation'] ] * self.ntrapezoid) )
                elif key == 'langles':
                    variations = self.xp.concatenate( (variations, [ initial_guess[key]['variation'] ]* self.ntrapezoid) )
...
class StackedTrapezoidDiffraction():
    ...
    def __init__(self, TrapezoidGeometry, xp=np):
        ...
        self.from_fitter = self.TrapezoidGeometry.from_fitter
    ...
    def correct_form_factor_intensity(self, qys, qzs, params=None, fitparams=None):
        ...
        if not self.from_fitter:
            ...
            params = self.StackedTrapezoidGeometry.check_params(params)
            height = params['heights']
            ...
            fitparams = fitparams * self.TrapezoidGeometry.variations + self.TrapezoidGeometry.initial_guess_values #make sure fitparams values make sense

            #get the indices to assign relevant values from fit_params
            if self.TrapezoidGeometry.fitparams_indices is None:
                self.TrapezoidGeometry.set_fitparams_indices()
                indices = self.TrapezoidGeometry.fitparams_indices
            ...
            weight = self.TrapezoidGeometry.initial_guess.get('weight', None)
...
TrapezoidGeometry = StackedTrapezoidGeometry(np, from_fitter=True, initial_guess=params1)

TrapezoidDiffraction = StackedTrapezoidDiffraction(TrapezoidGeometry, np)

corrected_intensity = TrapezoidDiffraction.correct_form_factor_intensity(qys=qxs, qzs=qzs, fitparams=fitparams)
